/*
 * APP_slave_Macros.h
 *
 * Created: 8/19/2023 11:24:00 PM
 *  Author: Dineth Jayakody
 */ 

#ifndef APP_SLAVE_MACROS_H_
#define APP_SLAVE_MACROS_H_
#include "STD_Types.h"

#define FAN_PIN						(uint8)0
#define ROOM1_PIN					(uint8)4
#define ROOM2_PIN					(uint8)5
#define ROOM3_PIN					(uint8)6
#define ROOM4_PIN					(uint8)7

#define FAN_PORT       				(uint8)'D'
#define ROOM1_PORT    				(uint8)'B'
#define ROOM2_PORT   				(uint8)'B'
#define ROOM3_PORT   				(uint8)'B'
#define ROOM4_PORT    				(uint8)'B'

#endif /* APP_SLAVE_MACROS_H_ */